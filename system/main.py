...
def run(args):
    ...
    for i in range(args.prev, args.times):
        ...
        if model_str == "mlr": # convex
            ...
            elif "eye" in args.dataset:
                args.model = FedAvgCNN(in_features=3, num_classes=4, dim=67500).to(args.device)
            else:
                args.model = FedAvgCNN(in_features=3, num_classes=args.num_classes, dim=10816).to(args.device)

        elif model_str == "dnn": # non-convex
            if "maternal_practical_noniid_unbalanced" in args.dataset:
                args.model = DNN().to(args.device)
        elif model_str=="log":
            if "diabetes_patho_noniid_unbalanced" in args.dataset:
                args.model=Mclr_Logistic().to(args.device)
            
        
        elif model_str == "resnet":
            args.model = torchvision.models.resnet18(pretrained=False, num_classes=4).to(args.device)
            ...
            args.model = alexnet(pretrained=False, num_classes=4).to(args.device)
        ...
        elif model_str == "efficientB0":
            args.model = torchvision.models.efficientnet_b0(pretrained=False,  num_classes=4).to(args.device)
            ...
            args.model = mobilenet_v2(pretrained=False, num_classes=4).to(args.device)
            ...
            if args.dataset == 'har_practical_non_iid_unbalanced':
                args.model = HARCNN(9, dim_hidden=1664, conv_kernel_size=(1, 9), pool_kernel_size=(1, 2)).to(args.device)
        ...
        if args.algorithm == "FedAvg":
            args.head = copy.deepcopy(args.model.fc4)
